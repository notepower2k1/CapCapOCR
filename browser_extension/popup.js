...
async function renderOverlay(tabId, phase2Payload, candidateId) {
  await runInTab(
    tabId,
    (payload, overlayRootId, overlayReaderId, selectedId) => {
      const showOverlayReader = (text) => {
        let reader = document.getElementById(overlayReaderId);
        if (!reader) {
          reader = document.createElement("div");
          reader.id = overlayReaderId;
          reader.style.position = "fixed";
          reader.style.left = "50%";
          reader.style.bottom = "24px";
          ...
          reader.addEventListener("click", () => reader.remove());
      ...
      for (const group of payload?.groups || []) {
        ...
        bubble.style.borderRadius = `${Math.max(
          6,
          Math.floor(Math.min(boxWidth, boxHeight) * 0.22)
        )}px`;
    ...
    [phase2Payload, OVERLAY_ROOT_ID, OVERLAY_READER_ID, candidateId]
  );
}
...
async function clearOverlay() {
  ...
  await runInTab(
    tab.id,
    (overlayRootId, overlayReaderId) => {
      document.getElementById(overlayRootId)?.remove();
      document.getElementById(overlayReaderId)?.remove();
    },
    [OVERLAY_ROOT_ID, OVERLAY_READER_ID]
  );
}
